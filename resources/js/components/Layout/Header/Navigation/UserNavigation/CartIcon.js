...
class CartIcon extends Component {
    ...
    render() {
        ...
        let itemsList = (
            <div className="border-b border-gray-light px-2 py-4 text-left text-sm">
        ...
        if (itemsInCart) {
            carticonOutput = (
                <span className="fa-layers fa-fw text-xl">
                    ...
                    <span className="fa-layers-counter fa-layers-top-right text-4xl">
                        {itemsInCart}
                    </span>
                </span>
            );
        ...
        // 					<p className="bg-gray-lighter text-xs p-2 w-auto">{item.qty} x</p>
        // 					<p className="text-sm">{item.name}</p>
        // 					<p className="text-sm text-gray-dark">size: {item.size}</p>
        ...
        return (
            <div className="group relative">
                ...
                <div className="absolute z-50 -ml-32 pin-r border border-gray-light bg-white invisible group-hover:visible w-64 h-auto">
                    <div className="bg-gray-lighter w-full h-8"></div>

                    {itemsList}

                    <div className="border-b border-gray-light p-3">
                        <div className="flex flex-col border-t border-gray-light">
                            <p className="py-2 text-black text-sm font-semibold">
                                Subtotal
                            </p>
                            <p className="py-2 text-xs">
                                (Including VAT and excluding delivery costs)
                            </p>
                            <p className="py-2 text-sm font-semibold text-right">
                                {priceFormat(displaySubTotal)}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="text-center py-3 px-6">
                        ...
                        <button
                            onClick={this.goToCartPage}
                            className="bg-transparent text-xs text-green-dark font-semibold py-2 px-4 border border-gray-dark w-full"
                        >
                            View shopping cart
                        </button>
                    </div>
